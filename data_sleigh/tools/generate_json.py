...
Examples:
    ...
    python generate_json.py /path/to/mqtt_logs.db water-level.json
"""
import argparse
import json
import sys
...
import duckdb
from data_sleigh.aggregator import (
    query_aggregated_data,
    query_water_levels,
    query_yolink_aggregated_data,
)
from data_sleigh.analyzer import analyze_water_level_segments
from data_sleigh.uploader import create_json_output
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="2024-12-01",
        help="Season start date (default: 2024-12-01)",
    )
    parser.add_argument(
        "--season-end",
        type=str,
        default="2025-01-15",
        help="Season end date (default: 2025-01-15)",
    )
    ...
    is_in_season = season_start <= today <= season_end

    # Query raw measurements
    print("  Querying raw measurements...", file=sys.stderr)
    measurements = query_water_levels(conn, minutes=args.minutes)
    ...
    try:
        agg_1m = query_aggregated_data(conn, interval_minutes=1, lookback_hours=1)
        agg_5m = query_aggregated_data(conn, interval_minutes=5, lookback_hours=24)
        agg_1h = query_aggregated_data(conn, interval_minutes=60, lookback_hours=None)
        print(
            f"  Water level: 1m={len(agg_1m)} pts, 5m={len(agg_5m)} pts, 1h={len(agg_1h)} pts",
            file=sys.stderr,
        )
    except Exception as e:
        print(
            f"  Warning: Could not generate water level aggregates: {e}",
            file=sys.stderr,
        )
    ...
    try:
        yolink_1m = query_yolink_aggregated_data(
            conn, interval_minutes=1, lookback_hours=1
        )
        yolink_5m = query_yolink_aggregated_data(
            conn, interval_minutes=5, lookback_hours=24
        )
        yolink_1h = query_yolink_aggregated_data(
            conn, interval_minutes=60, lookback_hours=None
        )
        ...
        if air_1m or water_1m or air_5m or water_5m or air_1h or water_1h:
            print(
                f"  YoLink: air(1m={air_1m}, 5m={air_5m}, 1h={air_1h}), water(1m={water_1m}, 5m={water_5m}, 1h={water_1h})",
                file=sys.stderr,
            )
        else:
    ...
    if not args.no_analysis:
        ...
        try:
            analysis = analyze_water_level_segments(conn)
            if analysis:
                ...
                if pred:
                    print(
                        f"  Current slope: {pred['slope_mm_per_hr']:.3f} mm/hr",
                        file=sys.stderr,
                    )
                    print(
                        f"  Time to refill: {pred['time_to_50mm_hours']:.1f} hours",
                        file=sys.stderr,
                    )
            else:
