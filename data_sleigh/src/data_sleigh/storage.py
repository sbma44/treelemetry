"""DuckDB storage backend for MQTT messages and YoLink sensor data."""
...
class MessageStore:
    """DuckDB-backed storage for MQTT messages and YoLink sensor data.
    ...
    Example:
        ...
        >>> store.create_yolink_table()
        >>> store.insert_yolink_message("air", "d88b4c04000a9da6", 72.5, 45.2, 100, -50, {...})
    """
    ...
    def create_yolink_table(self, table_name: str = "yolink_sensors") -> None:
        """Create a table for storing YoLink sensor data with normalized schema.

        Args:
            table_name: Name of the table to create (default: yolink_sensors)

        The table schema includes:
        - id: Auto-incrementing integer primary key
        - timestamp: Sensor reading timestamp
        - topic: Synthetic MQTT-style topic (yolink/{device_type}/{device_id})
        - device_id: YoLink device ID
        - device_type: Device type ('air' or 'water')
        ...
        - raw_json: Complete event data as JSON string (for debugging)
        """
        # Validate table name
        if not table_name.replace("_", "").isalnum():
            raise ValueError(f"Invalid table name: {table_name}")
        ...
        # Index on device for per-device queries
        self._conn.execute(
            f"""
        ...
        logger.info(f"YoLink table '{table_name}' ready with normalized schema")
    ...
    def insert_yolink_message(
        ...
        """Insert YoLink sensor data with normalized columns (batched).
        ...
        Args:
            ...
            battery: Battery level (0-100)
            signal: Signal strength (dBm)
            raw_data: Complete event data dictionary
            table_name: Target table name (default: yolink_sensors)
        """
        # Create synthetic topic for consistency with MQTT pattern
    ...
    def flush(self, table_name: str | None = None) -> None:
        ...
        for tbl in tables_to_flush:
            ...
            try:
                # Check if this is a YoLink table (has device_id field)
                if messages and "device_id" in messages[0]:
                    # YoLink table insert
                    self._conn.executemany(
                        f"""
                        INSERT INTO {tbl} (timestamp, topic, device_id, device_type,
                    ...
                    # Standard MQTT message table insert
                    self._conn.executemany(
                        f"""
                ...
                logger.debug(f"Flushed {len(messages)} messages to table '{tbl}'")
    ...
    def query(
        ...
        # Convert to dictionaries - get actual column names from result
    ...
    def get_connection(self) -> duckdb.DuckDBPyConnection:
        ...
        Note:
            Use this for complex queries like aggregations that need direct access.
        """
        return self._conn
