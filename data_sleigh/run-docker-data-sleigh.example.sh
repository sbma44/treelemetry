...
# MQTT Echo Configuration (relay ALL YoLink messages to local MQTT broker)
# Set MQTT_ECHO_BROKER to enable - messages are echoed to {topic_prefix}/{device_id}/report
MQTT_ECHO_BROKER="${MQTT_ECHO_BROKER:-}"
MQTT_ECHO_PORT="${MQTT_ECHO_PORT:-1883}"
MQTT_ECHO_USERNAME="${MQTT_ECHO_USERNAME:-}"
MQTT_ECHO_PASSWORD="${MQTT_ECHO_PASSWORD:-}"
MQTT_ECHO_TOPIC_PREFIX="${MQTT_ECHO_TOPIC_PREFIX:-yolink}"
...
if [ -n "$MQTT_ECHO_BROKER" ]; then
  echo "MQTT Echo: $MQTT_ECHO_BROKER:$MQTT_ECHO_PORT (prefix: $MQTT_ECHO_TOPIC_PREFIX)"
else
  echo "MQTT Echo: disabled"
fi
echo ""
...
docker run -d \
  ...
  -e MQTT_ECHO_BROKER="$MQTT_ECHO_BROKER" \
  -e MQTT_ECHO_PORT="$MQTT_ECHO_PORT" \
  -e MQTT_ECHO_USERNAME="$MQTT_ECHO_USERNAME" \
  -e MQTT_ECHO_PASSWORD="$MQTT_ECHO_PASSWORD" \
  -e MQTT_ECHO_TOPIC_PREFIX="$MQTT_ECHO_TOPIC_PREFIX" \
